...
(function() {
    ...
    function initScrollAnimations() {
        // Afficher immédiatement les éléments de la section financement
        const financementSection = document.querySelector('#financement');
        if (financementSection) {
            const financementElements = financementSection.querySelectorAll('.scroll-animate');
            financementElements.forEach(el => {
        ...
        // Vérifier le support de IntersectionObserver pour les autres sections
        ...
        // Créer l'observer pour les éléments hors section financement
        scrollObserver = new IntersectionObserver(handleIntersection, config);
        
        // Observer tous les éléments avec la classe scroll-animate (sauf section financement)
        const animatedElements = document.querySelectorAll('.scroll-animate');
        animatedElements.forEach(el => {
            // Exclure les éléments de la section financement
            const isInFinancementSection = el.closest('#financement');
            if (!isInFinancementSection) {
                scrollObserver.observe(el);
                
                // Ajouter un délai d'animation réduit pour un effet en cascade plus rapide
    ...
    function animateNumber(element) {
        ...
        function updateNumber(currentTime) {
            ...
            // Easing function améliorée (ease-out-quart pour plus de fluidité)
            const easeProgress = 1 - Math.pow(1 - progress, 4);
            
            // Utiliser Math.round au lieu de Math.floor pour une meilleure précision
            const currentValue = Math.round(easeProgress * numValue);
